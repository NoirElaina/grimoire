...
后端项目里用 Stream，先记住这几条：

1. `filter` 做筛选。
2. `map` 做对象转换。
3. `flatMap` 做扁平化。
4. `collect` 把流收回集合、Map 或分组结果。
5. `forEach` 主要做终止遍历，不建议在里面堆复杂业务。
6. `parallelStream` 不要随便用，尤其不要包数据库、HTTP、事务逻辑。
7. Stream 链太长就拆变量，不要为了“函数式”牺牲可读性。

一句话：

**Stream 最适合表达数据加工流水线，不适合藏复杂业务副作用。**
...
List<OrderVO> orderVOList = orders.stream()
        .map(order -> OrderVO.from(order, userMap.get(order.getUserId())))
        .toList();
```
...
只要 key 重复就可能抛异常。业务上允许重复时，要写 merge function。

### 3. 链太长

Stream 超过 5～6 步还夹着复杂 lambda，就该拆变量或抽方法。

### 4. 在 `peek` 里改状态

`peek` 不是业务操作入口。修改对象用 `map` 返回新对象，或者直接用循环。

### 5. 滥用 `parallelStream`

并行不是免费加速。后端 IO、事务和远程调用场景里，它经常是雷。

### 6. 忘记 null

Stream 链里任何一步拿到 null，都可能在后面炸。输入集合和关键字段都要有边界。
...
Java Stream 的价值不是“少写几行 for 循环”，而是：

**把集合数据的筛选、转换、分组和聚合表达成一条清晰的数据流水线。**
